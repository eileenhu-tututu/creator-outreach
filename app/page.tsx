...
import { ArrowDown, ArrowRight, Check, Clipboard, Code2, Copy, FileText, Mail, MessageCircle, RefreshCw, Send, ShieldCheck, Sparkles, Upload, Video, WandSparkles, X } from 'lucide-react';
...
export default function Home() {
  ...
  const collectYouTube = async () => {
    ...
    setCollecting(true); setCollectionMessage('Finding videos from the last 30 days…');
    try {
      const response = await fetch('/api/youtube', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ channel: channelName, maxVideos: 6 }) });
