    ...
    <main className="app-shell min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-[1440px] px-4 pb-8 pt-6 sm:px-5 lg:px-10 lg:pt-10">
        <div className="hero-studio">
          ...
          <div className="showcase-wrap" aria-label="Product workflow preview"><div className="showcase-live" aria-live="polite"><span><i /> LIVE WORKFLOW</span><strong>{['Reading creator videos','Ranking the product catalog','Building the email preview'][activeShowcase]}</strong></div><span className="float-emoji emoji-one" aria-hidden="true">✨</span><span className="float-emoji emoji-two" aria-hidden="true">🫶</span><span className="float-emoji emoji-three" aria-hidden="true">💌</span><div className="showcase-stage">
      ...
      <section id="generator" className="mx-auto max-w-[1440px] scroll-mt-24 px-5 py-10 lg:px-10">
        ...
        <div id="product-match" className="mt-12 scroll-mt-24">
          ...
          <article className="match-panel rounded-[28px] p-5 sm:p-7">
            <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div className="flex items-start gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-full bg-[#ff5400] text-black"><PackageSearch className="size-5" /></span><div><h3 className="text-xl font-black">Match creator signals to your catalog</h3><p className="mt-1 max-w-2xl text-sm leading-6 text-[#27322d]/60">Recent scripts and bio are compared with every product’s description and selling points. The strongest match is selected automatically, and you can switch to another candidate before writing.</p></div></div><Button onClick={matchProducts} disabled={matchingProducts || filledVideos === 0 || productLibrary.length === 0} className="match-primary-action h-12 shrink-0 rounded-full px-6 font-black">{matchingProducts ? <><RefreshCw className="animate-spin" /> Matching catalog…</> : <><PackageSearch /> {productMatches.length ? 'Re-match products' : 'Match products'}</>}</Button></div>
            {!productLibrary.length ? <div className="mt-6 rounded-[18px] border border-dashed border-black/15 p-6 text-center text-sm text-[#27322d]/55">Your product library is empty. <a href="/products" className="font-bold text-[#ff5400]">Add products first</a>.</div> : !productMatches.length ? <div className="mt-6 grid min-h-[150px] place-items-center rounded-[18px] border border-dashed border-black/12 bg-[#f4f8f1] text-center"><div><p className="font-bold">No match has been run yet.</p><p className="mt-1 text-sm text-[#27322d]/50">Add the creator’s scripts, then compare all {productLibrary.length} products.</p></div></div> : <div className="mt-7"><div className="grid gap-4 lg:grid-cols-3">{productMatches.slice(0, 3).map((match, index) => { const selected = selectedProductId === match.product.id; return <button key={match.product.id} onClick={() => chooseProduct(match)} className={`relative rounded-[22px] p-5 text-left transition ${selected ? 'bg-[#ff5400] text-black ring-2 ring-[#ff5400]' : 'bg-[#f4f8f1] text-[#27322d] ring-1 ring-black/10 hover:bg-[#e8f1e5]'}`}><div className="flex items-start justify-between gap-3"><div>{index === 0 && <Badge className="mb-3 rounded-full bg-black text-white">BEST MATCH</Badge>}<h4 className="text-xl font-black tracking-[-.03em]">{match.product.name}</h4></div><span className={`grid size-14 shrink-0 place-items-center rounded-full text-lg font-black ${selected ? 'bg-black text-white' : 'bg-white text-black shadow-sm'}`}>{match.score}%</span></div><p className={`mt-3 text-sm leading-6 ${selected ? 'text-black/65' : 'text-[#27322d]/60'}`}>{match.reason}</p><div className="mt-4 flex flex-wrap gap-2">{match.matchedFeatures.map((feature) => <Badge key={feature} className={`rounded-full ${selected ? 'bg-black text-white' : 'bg-[#e2ece0] text-[#27322d]'}`}>{feature}</Badge>)}</div><div className={`mt-5 flex items-center justify-between border-t pt-4 text-xs font-bold ${selected ? 'border-black/15' : 'border-black/10 text-[#27322d]/55'}`}><span>{match.product.commission} · {match.product.freeSample ? 'Free sample' : 'No sample'}</span><span>{selected ? 'SELECTED ✓' : 'SELECT'}</span></div></button>; })}</div>{!matchesFresh && <p className="mt-4 text-sm font-semibold text-[#b54c42]">Creator content or product library changed. Re-run matching before generating outreach.</p>}</div>}
          </article>
        </div>

        <div className="match-action-bar mt-5 flex flex-col items-center justify-between gap-4 rounded-[24px] p-5 sm:flex-row sm:px-7"><div className="flex items-center gap-3 text-sm text-[#27322d]/60"><span className="grid size-10 place-items-center rounded-full bg-[#e6efe3]"><ShieldCheck className="size-4 text-[#ff5400]" /></span><span><strong className="block text-[#27322d]">{selectedMatch && matchesFresh ? `${product} selected at ${selectedMatch.score}% match.` : 'Match a product before writing.'}</strong>{selectedMatch && matchesFresh ? selectedMatch.reason : 'The outreach will use only the selected product and traceable creator evidence.'}</span></div><Button onClick={() => generate()} disabled={generating || !username.trim() || !matchesFresh || !selectedMatch} className="h-14 w-full rounded-full bg-[#ff5400] px-8 text-base font-black text-black hover:bg-[#ff6a1a] sm:w-auto">{generating ? <><RefreshCw className="size-4 animate-spin" /> Writing outreach…</> : <><WandSparkles className="size-5" /> Generate with {selectedMatch && matchesFresh ? product : 'matched product'}</>}</Button></div>
      </section>
