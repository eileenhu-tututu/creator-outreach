...
export async function POST(request: Request) {
  ...
  const transcriptKey = requestCredential(
    request,
    ...
    return Response.json(
      {
        ...
        message:
          'Add YOUTUBE_API_KEY and SUPADATA_API_KEY to enable live collection.',
      },
      { status: 503 },
    );
  }
  ...
  if (!searchResponse.ok)
    return youtubeError(searchResponse, 'YouTube channel lookup failed.');
  ...
  if (!channelId)
    return Response.json(
      { error: 'No matching YouTube channel found.' },
      { status: 404 },
    );
  ...
  if (!channelResponse.ok)
    return youtubeError(channelResponse, 'Could not load the YouTube channel.');
  ...
  if (!uploadsId)
    return Response.json(
      { error: 'The channel upload list is unavailable.' },
      { status: 404 },
    );
  ...
  if (!videosResponse.ok)
    return youtubeError(
      videosResponse,
      'Could not load recent channel videos.',
    );
  const videosData = (await videosResponse.json()) as {
    items?: PlaylistVideo[];
  };
  const since = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const recent = (videosData.items || [])
    .filter((item) => new Date(item.snippet.publishedAt).getTime() >= since)
    .slice(0, maxVideos);
  ...
  const videos = await Promise.all(
    recent.map(async (item) => {
      ...
      const response = await fetch(transcriptUrl, {
        headers: { 'x-api-key': transcriptKey },
      });
      if (!response.ok) {
        return {
          videoId,
      ...
      return {
        ...
        url: videoUrl,
        transcript,
        status: data.jobId ? 'processing' : transcript ? 'ready' : 'failed',
        jobId: data.jobId,
      };
    }),
  );
  ...
  return Response.json({
    channel: {
      ...
      avatar: match.snippet.thumbnails?.default?.url,
    },
    rangeDays: 7,
    videos,
  });
